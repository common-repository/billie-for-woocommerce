...
class BillieBlocksSupport extends AbstractPaymentMethodType {
	...
	public function get_payment_method_script_handles(): array {
		$asset_path   = BILLIE_PLUGIN_URL . '/resources/build/index.asset.php';
		...
		wp_register_script(
			...
			BILLIE_PLUGIN_URL . '/resources/build/blocks.js',
			$dependencies,
			$version,
			true
		);

		wp_set_script_translations( 'wc-billie-blocks-integration', 'billie-for-woocommerce' );
	...
	public function get_payment_method_data(): array {
		...
		if ( $this->gateway ) {
			...
			$data['title']              = __( 'Pay After Delivery', 'billie-for-woocommerce' );
			$data['method_description'] = __( 'Billie Description', 'billie-for-woocommerce' );

			if ( isset( $this->gateway->settings['description'], $this->gateway->settings['payment_term'] ) ) {
				$data['method_description'] = str_replace( ['{Zahlungsziel}', '{payment-goal}'], $this->gateway->settings['payment_term'], $this->gateway->settings['description'] );
			}
