...
class PaymentInfo {
        ...
        <section class="woocommerce-order-details billie-payment">
            <p>
				...
				if ( ! $metabox && null !== $payment_info['paymentGoal'] ) {
					echo esc_html(
						sprintf(
							__( 'Please make your payment to the following bank account until %s while providing the invoice number:', 'billie-for-woocommerce' ),
				...
				echo esc_html(
					sprintf(
						__( 'Account owner: %s', 'billie-for-woocommerce' ),
				...
				echo esc_html(
					sprintf(
						__( 'IBAN: %s', 'billie-for-woocommerce' ),
						$payment_info['iban']
					)
				);
				?><br>
				...
				echo esc_html(
					sprintf(
						__( 'BIC: %s', 'billie-for-woocommerce' ),
						$payment_info['bic']
					)
				);
				?><br>
				...
				echo esc_html(
					sprintf(
						__( 'Bank: %s', 'billie-for-woocommerce' ),
						$payment_info['bank']
					)
				);
				?><br>
				...
				if ( null !== $payment_info['paymentGoal'] ) {
					echo esc_html(
						sprintf(
							__( 'Payment goal: %s', 'billie-for-woocommerce' ),
							$payment_info['paymentGoal']
						)
					);
				} else {
					esc_html_e( 'Order not yet shipped', 'billie-for-woocommerce' );
				}
				?><br>
				...
				echo esc_html(
					sprintf(
						__( 'Subject: %s', 'billie-for-woocommerce' ),
	...
	public static function get_bank( string $bic ) {
		...
		if ( ! file_exists( BILLIE_RESOURCES_PATH . '/bic-de.csv' ) ) {
			return $bic;
		}

		$bic_csv = fopen( BILLIE_RESOURCES_PATH . '/bic-de.csv', 'rb' );
