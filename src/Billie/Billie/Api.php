...
class Api {
	...
	public function validate_credentials( $billie_client_id, $billie_client_secret, $is_sandbox ): void {
		...
		if ( ! isset( $validationResult['client_id'] ) ) {
			throw new BillieException( __( 'Unexpected validation format', 'billie-for-woocommerce' ) );
		}

		if ( $validationResult['client_id'] !== $billie_client_id ) {
			throw new BillieException( __( 'Unexpected validation client id', 'billie-for-woocommerce' ) );
		}
	}
	...
	public function get_billie_session_id( $wooCommerceSessionId ): string {
		...
		if (
			...
			throw new CredentialsNotSetException( __( 'Missing Credentials', 'billie-for-woocommerce' ) );
		}
