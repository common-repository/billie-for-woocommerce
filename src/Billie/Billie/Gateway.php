...
class Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->title              = __( 'Pay After Delivery', 'billie-for-woocommerce' );
		$this->method_title       = __( 'Billie', 'billie-for-woocommerce' );
		$this->method_description = __( 'Billie Description', 'billie-for-woocommerce' );
		...
		if ( isset( $this->settings['description'], $this->settings['payment_term'] ) ) {
			$this->method_description = str_replace( ['{Zahlungsziel}', '{payment-goal}'], $this->settings['payment_term'], $this->settings['description'] );
		}
	...
	public function init_form_fields(): void {
		$this->form_fields = [
			'enabled'               => [
				'title'   => __( 'Enable/Disable', 'billie-for-woocommerce' ),
				'type'    => 'checkbox',
				'label'   => __( 'Enable this payment method', 'billie-for-woocommerce' ),
				'default' => 'no',
			],
			'title'                 => [
				'title'       => __( 'Title', 'billie-for-woocommerce' ),
				'type'        => 'text',
				'description' => __( 'This controls the title which the user sees during checkout.', 'billie-for-woocommerce' ),
				'default'     => __( 'Billie BNPL B2B', 'billie-for-woocommerce' ),
				'desc_tip'    => true,
			],
			'payment_term'          => [
				'title'       => __( 'Payment Term', 'billie-for-woocommerce' ),
				'type'        => 'integer',
				'description' => __( 'Based upon your Billie contract, your customers will have between 7 and 120 days (payment term) to pay your invoices.', 'billie-for-woocommerce' ),
				'default'     => 30,
				'desc_tip'    => true,
			],
			'capture_target_status' => [
				'title'       => __( 'Authorization & Capture', 'billie-for-woocommerce' ),
				'type'        => 'select',
				'options'     => [
					'wc-processing' => 	__( 'Authorize and capture on processing status', 'billie-for-woocommerce' ),
					'wc-completed' => 	__( 'Authorize on processing and capture on completed', 'billie-for-woocommerce' ),
				],
				'default'     => self::CAPTURE_TARGET_STATUS_DEFAULT,
			],
			'hide_logo'             => [
				'title' => __( 'Hide Logo', 'billie-for-woocommerce' ),
				'type'  => 'checkbox',
				'label' => __( 'Hide Billie Logo', 'billie-for-woocommerce' ),
			],
			'description'           => [
				'title'   => __( 'Customer Message', 'billie-for-woocommerce' ),
				'type'    => 'textarea',
				'default' => __( 'Pay within {payment-goal} days after receiving your order - convenient and secure by invoice.', 'billie-for-woocommerce' ),
			],
			'billie_sandbox_or_production' => [
				'title' => __( 'Sandbox or production', 'billie-for-woocommerce' ),
				'type'  => 'select',
				'options' => [
					'sandbox'    => __( 'Sandbox', 'billie-for-woocommerce' ),
					'production' => __( 'Production', 'billie-for-woocommerce' ),
				],
				'default' => 'sandbox',
			],
			'billie_client_id' => [
				'title' => __( 'Client ID', 'billie-for-woocommerce' ),
				'type' => 'text',
			],
			'billie_client_secret' => [
				'title' => __( 'Client Secret', 'billie-for-woocommerce' ),
				'type' => 'text',
			],
		];
			...
			if ( isset( $settings['billie_sandbox_or_production'] )
				...
				try {
					...
					\WC_Admin_Settings::add_error(
						sprintf(
							__( 'The Billie client credentials (ID and Secret) are not valid. Without valid credentials, this payment method can not be used. (Billie-API responded: %s)', 'billie-for-woocommerce' ),
							$e->getMessage()
						)
					);
				}
	...
	public function process_decline( string $billie_session_id ): void {
		...
		try {
			...
			wc_add_notice( __( 'Payment declined.', 'billie-for-woocommerce' ), 'error' );
			$order->update_status( 'failed', __( 'Payment declined.', 'billie-for-woocommerce' ) );
		} catch ( \Exception $e ) {
			wc_add_notice( __( 'Payment failed.', 'billie-for-woocommerce' ), 'error' );
			$order->update_status( 'failed', __( 'Payment failed.', 'billie-for-woocommerce' ) );
		}
	...
	public function process_accept( string $billie_session_id ): void {
		$order_id = $this->get_order_id_for_billie_session_id( $billie_session_id );
		if ( ! $order_id ) {
			wc_add_notice( __( 'Order not found.', 'billie-for-woocommerce' ), 'error' );
			wp_redirect( wc_get_checkout_url() );
			exit;
		}
		$order = wc_get_order( $order_id );
		...
		if ( $authorization_data['state'] !== 'authorized' ) {
			wc_add_notice( __( 'Payment not authorized.', 'billie-for-woocommerce' ), 'error' );
			wp_redirect( wc_get_checkout_url() );
		}
		...
		$order->add_order_note(
			sprintf(
				__( '[Billie] Order has been created: %s', 'billie-for-woocommerce' ),
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		if ( (float) $amount <= 0.0 ) {
			return new \WP_Error( 1, __( 'Debit amount must be greater than zero.', 'billie-for-woocommerce' ) );
		}
	...
	public function validate_fields() {
		...
		if ( empty( $billing_company ) ) {
			wc_add_notice( __( 'This payment option is only available if the billing address has a value in the "company" field.', 'billie-for-woocommerce' ), 'error' );
	...
	public function order_updated( $order_id, $order ): void {
		...
		if ( $billie_order_id ) {
			...
			if ( ! $billie_invoice_uuid ) {
				...
				$order->add_order_note(
					sprintf(
						__( '[Billie] Order %s has been updated', 'billie-for-woocommerce' ),
						$billie_order_id
					)
				);
			}
	...
	private function cancel_order( WC_Order $order ): void {
		...
		$order->add_order_note(
			sprintf(
				__( '[Billie] Order %s has been deleted', 'billie-for-woocommerce' ),
				$billie_order_id
			)
		);
	}
	...
	private function create_credit_note( WC_Order $order, $refund ): void {
		...
		if ( ! $billie_invoice_id ) {
			throw new NoInvoiceFoundException( __( 'This order has not been captured yet.', 'billie-for-woocommerce' ) );
		}
		...
		$order->add_order_note(
			sprintf(
				__( '[Billie] Credit note for invoice %s has been created: %s', 'billie-for-woocommerce' ),
				$billie_invoice_id,
				$response['uuid']
			)
		);
	...
	private function create_invoice( WC_Order $order ): void {
		...
		if ( trim( (string) $invoice_id ) === '' ) {
			...
			throw new BillieException( __( 'Invoice ID not set', 'billie-for-woocommerce' ) );
		}
		...
		$order->add_order_note(
			sprintf(
				__( '[Billie] Invoice has been created: %s', 'billie-for-woocommerce' ),
				$response['uuid']
			)
		);
	}
	...
	private function cancel_invoice( $order ): void {
		...
		$order->add_order_note(
			sprintf(
				__( '[Billie] Invoice %s has been deleted', 'billie-for-woocommerce' ),
				$billie_invoice_uuid
			)
		);
	}
